...
def token(text):
    X = tokenizer(text,truncation=True, padding=True)
    return X
...
# df_train_subset['text'] = df_train_subset['text'].apply(lambda x: token(x))

# print(len(df_train_subset['text']))
# print(df_train_subset['text'][0:2])
...
from sklearn.metrics import roc_auc_score, f1_score, classification_report,accuracy_score,confusion_matrix, precision_score, recall_score
from sklearn.metrics import ConfusionMatrixDisplay

MODEL_NAME = "xlnet-base-cased"
# classifier = TFXLNetForSequenceClassification.from_pretrained(MODEL_NAME)#chose the optimizer
...
# loss = model(X_train, labels=y_train)
